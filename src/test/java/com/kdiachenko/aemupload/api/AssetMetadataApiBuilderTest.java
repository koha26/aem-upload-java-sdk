...
class AssetMetadataApiBuilderTest {
    ...
    void build_shouldCreateApiWithDefaultPathNormalizer() {
        AssetMetadataApi api = AssetMetadataApiBuilder.builder(apiServerConfiguration)
    ...
    void builder_shouldReturnSameInstanceForChaining() {
        AssetMetadataApiBuilder builder = AssetMetadataApiBuilder.builder(apiServerConfiguration);

        AssetMetadataApiBuilder result = builder.withPathNormalizer(path -> path);
